...
```ts
import logo from "@wardnet/brand/svg/wardnet-logo-dark.svg";
import mark from "@wardnet/brand/svg/wardnet-mark-emerald.svg";
```
...
```sh
yarn wardnet-brand-png node_modules/@wardnet/brand/assets/dist/wardnet-icon.svg 192
```

Regenerate the outlined `dist/` from the text sources (maintainers, after editing `assets/src/`):

```sh
yarn workspace @wardnet/brand build   # outlines <text> → <path> using the brand fonts
```
